...
app.post("/orders", requireAuth, async (req, res) => {
  try {
    ...
    await orderService.notifyUser(
      order.id,
      req.session.user.id,
      `OS ${order.os_number} aberta`,
      body,
    );
    await orderService.addOrderEvent(
      order.id,
      req.session.user.id,
      "created",
    ...
    for (const admin of admins) {
      ...
      try {
        ...
        console.error(
          "Falha ao enviar WhatsApp para admin na abertura da OS:",
          error.message,
        );
      }
    }
    ...
    if (author?.phone) {
      try {
        ...
        console.error(
          "Falha ao enviar WhatsApp para autor da OS na abertura:",
          error.message,
        );
      }
    }
    ...
    res
      .status(500)
      .render("orders-new", {
        error: "Não foi possível abrir a ordem de serviço.",
      });
  }
});
...
app.post("/orders/:id/comments", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (status && status !== order.status) {
      await db.execute("UPDATE service_orders SET status = ? WHERE id = ?", [
        status,
        req.params.id,
      ]);
      ...
      await orderService.notifyAdmins(
        req.params.id,
        `OS ${order.os_number} atualizada`,
        `Status atualizado para ${res.locals.statusLabel(status)} na OS ${order.os_number}.`,
      );
      if (order.opened_by_user_id) {
        await orderService.notifyUser(
          req.params.id,
          order.opened_by_user_id,
          `OS ${order.os_number} atualizada`,
          `Status atualizado para ${res.locals.statusLabel(status)} na OS ${order.os_number}.`,
        );
      }
    ...
    if (comment) {
      ...
      await orderService.notifyAdmins(
        req.params.id,
        `Comentário na OS ${order.os_number}`,
        commentMessage,
      );
      if (order.opened_by_user_id) {
        await orderService.notifyUser(
    ...
    for (const admin of admins) {
      ...
      try {
        ...
        if (status && status !== order.status)
          lines.push(`Novo status: ${res.locals.statusLabel(status)}`);
        ...
        console.error(
          "Falha ao enviar WhatsApp para admin na atualização da OS:",
          error.message,
        );
      }
    }
    ...
    if (author?.phone) {
      try {
        ...
        console.error(
          "Falha ao enviar WhatsApp para autor na atualização da OS:",
          error.message,
        );
      }
    }
...
app.post("/admin/integrations", requireAdmin, async (req, res) => {
  try {
    ...
    const apiToken =
      keepApiToken && existing?.api_token
        ? existing.api_token
        : existing?.api_token || integrationService.generateToken();
...
app.post("/admin/users/:id/delete", requireAdmin, async (req, res) => {
  ...
  if (user.role === "admin") {
    return res
      .status(400)
      .send("Usuários administradores não podem ser excluídos.");
  }
...
app.post("/api/webhook/whatsapp/os", async (req, res) => {
  try {
    ...
    const requesterPhone = String(contactPhone || "").trim();
    const account = await authService.createUserIfMissing({
      name: requesterName,
    ...
    if (account.created && account.user?.phone) {
      try {
        ...
        console.error(
          "Falha ao enviar mensagem de acesso para novo usuário:",
          error.message,
        );
      }
    }
